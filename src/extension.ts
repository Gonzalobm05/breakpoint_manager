...
export function activate(context: vscode.ExtensionContext) {
  ...
  const treeView: vscode.TreeView<vscode.TreeItem> =
    vscode.window.createTreeView("breakpointGroups", {
  ...
  context.subscriptions.push(
    treeView,
    vscode.window.registerTreeDataProvider("breakpointGroups", provider),
    vscode.commands.registerCommand("breakpointGroups.addGroup", () =>
      provider.addGroup()
    ),
    vscode.commands.registerCommand("breakpointGroups.toggleGroup", (item) =>
      provider.toggleGroup(item)
    ),
    vscode.commands.registerCommand("breakpointGroups.renameGroup", (item) =>
      provider.renameGroup(item)
    ),
    vscode.commands.registerCommand("breakpointGroups.deleteGroup", (item) =>
      provider.deleteGroup(item)
    ),
    vscode.commands.registerCommand("breakpointGroups.addBreakpoint", (item) =>
      provider.addBreakpointToGroup(item)
    ),
    vscode.commands.registerCommand(
      "breakpointGroups.deleteBreakpoint",
      (item) => provider.deleteBreakpoint(item)
    ),
    vscode.commands.registerCommand("breakpointGroups.moveBreakpoint", (item) =>
      provider.moveBreakpoint(item)
    ),
    vscode.commands.registerCommand("breakpointGroups.selectGroup", (item) =>
      provider.selectGroup(item)
    )
  );
  vscode.debug.onDidChangeBreakpoints((event) => {
    provider.handleNewBreakpoints(event);
  });
  let lastSelectedItem: vscode.TreeItem | null = null;
...
{
  private groups: { [name: string]: vscode.TreeItem[] } = {};
  private _onDidChangeTreeData: vscode.EventEmitter<vscode.TreeItem | void> =
    new vscode.EventEmitter();
  readonly onDidChangeTreeData: vscode.Event<vscode.TreeItem | void> =
    this._onDidChangeTreeData.event;

  private suppressHandleNewBreakpoints: boolean = false;
  private context: vscode.ExtensionContext;
  private selectedGroup: string | null = null;
  private groupToggleState: { [groupName: string]: boolean } = {};
  readonly dropMimeTypes = ["application/vnd.code.tree.breakpointGroups"];
  readonly dragMimeTypes = ["application/vnd.code.tree.breakpointGroups"];

  constructor(context: vscode.ExtensionContext) {
    this.context = context;
    this.loadGroups();
    this.groupToggleState = this.context.globalState.get<{
  ...
  async handleDrop(
    ...
    draggedItems: vscode.DataTransfer,
    token: vscode.CancellationToken
  ) {
    const groupName = target.label?.toString() ?? "";
    if (!this.groups[groupName]) return; // Ensure valid target group

    const draggedItem = draggedItems.get(
      "application/vnd.code.tree.breakpointGroups"
    );
    if (!draggedItem) return;

    const movedBreakpointLabel = draggedItem.value;
    let movedBreakpoint: vscode.SourceBreakpoint | undefined;
  ...
  async handleDrag(
    ...
    if (source.length === 0 || source[0].contextValue !== "breakpoint") return;

    const draggedItemLabel =
      typeof source[0].label === "string"
        ? source[0].label
        : source[0].label?.label ?? "";
    dataTransfer.set(
      "application/vnd.code.tree.breakpointGroups",
      new vscode.DataTransferItem(draggedItemLabel)
    );

    vscode.window.showInformationMessage(
      `Dragging breakpoint: ${draggedItemLabel}`
    );
  }
    ...
    if (!element) {
      ...
      return sortedGroups.map((groupName) => {
        ...
        groupItem.iconPath = new vscode.ThemeIcon(
          this.groupToggleState[groupName] ? "circle-large-filled" : "circle-large-outline"
        );
        return groupItem;
      });
    } else {
      ...
      return (this.groups[groupName] ?? [])
        .sort((a, b) =>
          this.getBreakpointLabel(a as vscode.SourceBreakpoint).localeCompare(
            this.getBreakpointLabel(b as vscode.SourceBreakpoint)
          )
        )
        .map((bp) => {
          if (bp instanceof vscode.SourceBreakpoint) {
            const bpItem = new vscode.TreeItem(
  ...
  handleNewBreakpoints(event: vscode.BreakpointsChangeEvent) {
    ...
    event.added.forEach((bp) => {
      if (bp instanceof vscode.SourceBreakpoint) {
        ...
        if (
          !this.groups[groupName].some(
            (existingBp) =>
              existingBp instanceof vscode.SourceBreakpoint &&
  ...
  selectGroup(item: vscode.TreeItem) {
  ...
  unselectGroup() {
    ...
    vscode.window.showInformationMessage(
      "No group selected. Breakpoints will be added to 'Ungrouped'."
    );
  }

  private getBreakpointLabel(bp: vscode.SourceBreakpoint): string {
  ...
  renameGroup(item: vscode.TreeItem) {
    const oldGroupName =
      typeof item.label === "string" ? item.label : item.label?.label ?? "";
    if (!this.groups[oldGroupName]) return;
    ...
    vscode.window
      ...
      .then((newGroupName) => {
        ...
        this._onDidChangeTreeData.fire();
        vscode.window.showInformationMessage(
          `Renamed group "${oldGroupName}" to "${newGroupName}".`
        );
      });
  }

  deleteGroup(item: vscode.TreeItem) {
    const groupName =
      typeof item.label === "string" ? item.label : item.label?.label ?? "";
    ...
    vscode.window.showInformationMessage(
      `Deleted group "${groupName}". Breakpoints moved to "Ungrouped".`
    );
  }

  addBreakpointToGroup(item?: vscode.TreeItem) {
    ...
    vscode.window
      .showQuickPick(
        breakpoints.map((bp) => this.getBreakpointLabel(bp)),
        { placeHolder: "Select Breakpoint to Add" }
      )
      .then((selected) => {
        if (selected) {
          const selectedBreakpoint = breakpoints.find(
            (bp) => this.getBreakpointLabel(bp) === selected
          );
          if (selectedBreakpoint) {
            // Determine the target group
            const groupName =
              item?.label?.toString() ?? this.selectedGroup ?? "Ungrouped";
  ...
  deleteBreakpoint(item: vscode.TreeItem) {
    for (const group in this.groups) {
      this.groups[group] = this.groups[group].filter((bp) => {
        if (bp instanceof vscode.SourceBreakpoint) {
  ...
  moveBreakpoint(item: vscode.TreeItem) {
    vscode.window
  ...
  toggleGroup(item: vscode.TreeItem) {
    const groupName =
      typeof item.label === "string" ? item.label : item.label?.label ?? "";
    if (!this.groups[groupName]) return;
    ...
    const oldBreakpoints = this.groups[groupName].filter(
      (bp) => bp instanceof vscode.SourceBreakpoint
    ) as vscode.SourceBreakpoint[];
    ...
    const updatedBreakpoints = oldBreakpoints.map(
      (bp) =>
        new vscode.SourceBreakpoint(
          bp.location,
          this.groupToggleState[groupName]
        )
    );
    ...
    vscode.window.showInformationMessage(
      `Breakpoints in "${groupName}" are now ${
        this.groupToggleState[groupName] ? "ENABLED" : "DISABLED"
      }.`
    );
  ...
  private saveGroups() {
    const serializedGroups = Object.keys(this.groups).reduce(
      (acc, groupName) => {
        acc[groupName] = this.groups[groupName]
          .filter((bp) => bp instanceof vscode.SourceBreakpoint) // Only store valid breakpoints
          .map((bp) => ({
            uri: (bp as vscode.SourceBreakpoint).location.uri.toString(),
            line: (bp as vscode.SourceBreakpoint).location.range.start.line,
            enabled: (bp as vscode.SourceBreakpoint).enabled, // Store enabled state
          }));
        return acc;
      },
      {} as {
        [name: string]: { uri: string; line: number; enabled: boolean }[];
      }
    );
  ...
  private loadGroups() {
    const savedData = this.context.globalState.get<string>(
      "breakpointGroups",
      "{}"
    );
    try {
      ...
      this.groups = Object.keys(parsedData).reduce((acc, groupName) => {
        acc[groupName] = parsedData[groupName].map(
          (bpData: { uri: string; line: number; enabled: boolean }) => {
            const bp = new vscode.SourceBreakpoint(
              new vscode.Location(
                vscode.Uri.parse(bpData.uri),
                new vscode.Position(bpData.line, 0)
              ),
              bpData.enabled
            );
        ...
        return acc;
      }, {} as { [name: string]: vscode.SourceBreakpoint[] });
